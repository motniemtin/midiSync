...
Lato.loadFont();
export const TitleScene = ({title, author}) => {
    const { width } = useVideoConfig();
    const { fps } = useVideoConfig();
    const marginBottom = width * 0.1;
    const frame = useCurrentFrame();    
    const translateX = useTranslate({ from: 0, to: 0, startAtFrame: fps });
    const translateY = useTranslate({ from: 150, to: -1100, startAtFrame: fps });
    const scale = useTranslate({ from: 1, to: 0.3, startAtFrame: fps * 0.95 });
    ...
    return (
        <Container $marginBottom={marginBottom} $scale={scale}>
            <TextContainer $translateX={translateX} $translateY={translateY}>
                ...
                {author && <Author as="h2" $opacity={opacity}>{author}</Author>}
            </TextContainer>
        </Container>
    );
};
